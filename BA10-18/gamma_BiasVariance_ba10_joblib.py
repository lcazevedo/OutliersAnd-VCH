...
def main_sequence():
    ...
    #ds_ba10 <- read_ba10
    ds_ba10, dataBA10, folds, new_runtimes = read_dataset(log, config)
    runtimes.update(new_runtimes)
    log.info(f"Dataset was mounted in {runtimes['read_ba10']}.")
    ...
    decomposition, average_vch, new_runtimes = kfold_bias_variance_decomp(log, config, model, ds_ba10, 
    ...
    folds_processed, decomposition, x_average_vch = read_folds_results(config, folds, ds_ba10)
    if len(folds_processed) == config[config["test_ref"]]["num_folds"]:
        # statistical analysis
        statistics = statistical_analysis(config, decomposition, dataBA10)
        log.info("Statistics has been calculated.")

        runtimes["total run time"] = str(datetime.now() - t0)
        
        #save_results    
        save_results(log, decomposition, dataBA10, average_vch, runtimes, statistics, config)
    else:
...
def statistical_analysis(config, decomposition, dataBA10):
    y = dataBA10[config[config["test_ref"]]["column_y"]]
...
def read_dataset(log, config):
    ...
    fileBA10 = os.path.join(config["path"], config["fileBA10"])

    xyz = qmmlpack.import_extxyz(fileBA10, additional_properties=True, output='raw')
    #Interface do xyz:
    #na   -- number of atoms
    #an   -- atomic numbers
    ...
    ds = Dataset(
        ...
        b=_addp,
        name="ba10",
        p={"te": _mp[:,2].astype(float), "VCH": np.array(_vch)},
    )
    
    # Same data as dataframe 
    dataBA10 = pd.DataFrame({'name': _mp[:,0], "te": _mp[:,2].astype(float), "number_of_atoms": _na, 
                        "coordinates": _xyz, "VCH": _vch})
    ...
    if os.path.isfile(file_name):
        ...
        folds = {}
        count_fold = 1
        for train, test in kf.split(ds.p["te"]):
            folds[f"train_{count_fold}"] = train.tolist()
            folds[f"test_{count_fold}"] = test.tolist()
            count_fold = count_fold + 1
        save_yaml(file_name, folds)

    period = {"read_ba10": str(datetime.now() - t0)}
    
    return ds, dataBA10, folds, period

def read_model(test_config):
    
    return Model.from_config(test_config["model"])
    
def save_results(log, decomposition, dataBA10, average_vch, runtimes, statistics, config):
    ...
    file_name = os.path.join(config["path"], "ba10_estudo_erro_"+config["test_ref"]+".pkl")
    data2.to_pickle(file_name)
    file_name_csv = os.path.join(config["path"], "ba10_estudo_erro_"+config["test_ref"]+".csv")
